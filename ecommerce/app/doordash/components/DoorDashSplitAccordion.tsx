...
export interface DoorDashAccordionItem {
  title: string;
  description: string;
  href?: string;
  /** Image shown on the right panel while this item is expanded */
  image: string;
  /** SVG path "d" attribute(s) for the item's icon, rendered in a 24x24 viewBox */
  iconPaths?: string[];
}

interface DoorDashSplitAccordionProps {
  items: DoorDashAccordionItem[];
}
...
function AccordionIcon({ paths }: { paths: string[] }) {
  return (
...
export function DoorDashSplitAccordion({ items }: DoorDashSplitAccordionProps) {
      ...
      <div className="grid gap-8 md:grid-cols-2 md:items-stretch">
        <div className="flex flex-col">
          {items.map((item, index) => {
            ...
            return (
              <div key={item.title} className="flex flex-col border-t border-[var(--dd-input-border)]">
                <button
                  type="button"
                ...
                >
                  <span className="flex items-center gap-3">
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center text-[var(--dd-input-border)]">
                      <AccordionIcon paths={item.iconPaths ?? defaultIconPaths} />
                    </span>
                    <span className="dd-accordion-title">{item.title}</span>
                  </span>
                  <span
                    className="flex h-4 w-4 shrink-0 items-center justify-center text-[var(--dd-input-border)] transition-transform duration-200"
                ...
                {isOpen && (
                  <div className="flex flex-col gap-4 px-2 pb-4">
                    <p className="text-[14px] font-medium leading-[21px] text-[var(--dd-text-muted)]">
                      {item.description}
                    </p>
                    {item.href && (
                      <a href={item.href} className="dd-pill-link w-fit">
                        Learn more
                      </a>
                    )}
                  </div>
        ...
        <div className="md:sticky md:top-24 md:h-full md:min-h-[420px]">
